...
namespace cmdLine {
	...
	*/
	...
	public:
		...
		*/
		void ArgControl(ArgStore & argStore) const {
			if (count > 3) {
				argStore.inputDataset = checkInput();
				argStore.outputDataset = checkOutput();
				argStore.myK = checkK();
			}
			else {
				cout << "Error." << endl;
